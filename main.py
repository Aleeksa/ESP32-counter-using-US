...
# Inicijalizacija LED diode i zujalice
red_led = Pin(2, Pin.OUT)  # Promenite pin prema vašem rasporedu
buzzer = Pin(15, Pin.OUT)  # Promenite pin prema vašem rasporedu
...
DETECTION_DELAY = 45000  # 45 sekundi u milisekundama
...
def start_web_server():
    global object_count  # Dodajemo global da možemo menjati vrednost
    ...
    while True:
        ...
        if 'GET /count' in request:
            ...
            update_display(0, object_count)  # Resetuj prikaz na LCD-u
            response = f"Counter reset to {object_count}"
            conn.send('HTTP/1.1 200 OK\n')
            conn.send('Content-Type: text/plain\n')
            conn.send('Connection: close\n\n')
            conn.sendall(response)
...
while True:
    ...
    if current_state and not previous_state and direction > 0:
        if time.ticks_diff(current_time, last_detection_time) > DETECTION_DELAY:
            ...
            # Aktiviraj LED diodu i zujalicu
            red_led.on()
            buzzer.on()
            time.sleep(3)  # Zujalica radi 3 sekunde
            buzzer.off()

    # Provera trajanja svetljenja LED diode
    if time.ticks_diff(current_time, last_detection_time) > DETECTION_DELAY:
        red_led.off()
